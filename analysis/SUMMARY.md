...
**2-C. 종목별 — 공포에 사면 득일까? (공정 비교)**
- 공포구간 = VIX 25 돌파 ~ 20 회복 (1년 3회·총 **29거래일**).
- ⚠️ "공포에 판 뒤 평소(214일) 보유"를 1년으로 깔면 +317%로 보이지만, 이는 공포구간(29일)보다 **7.4배 오래 시장에 머문 '보유시간 효과'**일 뿐 매도 타이밍의 우월성이 아니다.
- **공정 비교 = 같은 공포구간 안에서** '보유(공포에 산)' vs '현금(공포에 판=0%)':

| 종목 | 공포구간 보유(공포에 산) | 현금(공포에 판) |
|------|---:|---:|
| 삼성전자 | +7.4% | 0% |
| SK하이닉스 | +5.3% | 0% |
| HD한국조선해양 | −5.5% | 0% |
...
![공정 비교: 공포구간 vs 평소](charts/81_fair_compare.png)

→ 이 1년엔 공포구간이 짧고 약세라 "공포에 보유"가 바스켓 기준 **약 2%p 손해**(반도체만 +5~7% 플러스). 단 **차이는 미미**(±몇 %). 하루당 수익률도 공포구간(바스켓 −0.08%/일) < 평소(+0.53%/일) → 상승은 잔잔한 평소 구간에 집중. 공포 타이밍의 우열은 사실상 없었고, 결국 **"안 팔고 오래 보유"가 수익의 핵심.**
