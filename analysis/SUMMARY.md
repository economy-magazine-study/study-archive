...
> ⚠️ **매도 시점 주의**: 검증 1~6의 매도일은 전부 **2026-06-16(현재)** — 즉 "신호일 매수 후 **현재까지 보유(buy & hold)**". 기사의 "반등 시 매도"는 **검증 7**에서 별도 구현.

### 검증 0 — 신호는 실제 발생했나? ✅
- **VIX 30 돌파(연중 최고 31.0)·HY 최대 신용공포(−3.2%) 모두 2026-03-27.**
- "세 지표 동시 충족"이 3월 말 실제 발생, 그 지점이 거의 정확히 **단기 저점**.

![코어 바스켓 vs KOSPI 추이](charts/30_backtest_value.png)

### 검증 1 — "공포에 사면?" (코어 5종목 동일가중 → 6/16)
...
→ **통했다.** 최악 타이밍도 +35%. 분할매수는 고점·저점 중간 → "타이밍 위험 축소" 논리 확인.
...
→ 공포 매도자는 **약 188%p 상실.** "코어 사수"가 최대 교훈.

![패닉셀 vs 보유 기회손실](charts/32_backtest_panic.png)

### 검증 3 — VIX≥25 신호 7회 × 종목 평균 (신호일→6/16)
| 종목 | 평균 | 비고 |
|------|---:|------|
| SK하이닉스 | **+211%** | 전 신호 1위 |
| 삼성전자 | +124% | 반도체 강세 |
...
### 검증 7 — ★ 핵심: "반등 시 매도"를 실제 적용하면? (왕복매매)
- **규칙**: 매수 = VIX 임계 상향돌파 종가 / 매도 = 이후 **VIX<20(정상화)** 첫 종가. 매도 후 다음 공포까지 현금.

**전략 A — 매수 VIX≥25 → 매도 VIX<20 (3회 왕복)**
| 매수일 (VIX) | 매도일 (VIX) | 보유 | 바스켓 |
|------|------|---:|---:|
| 2025-10-16 (25) | 2025-10-20 (18) | 4일 | +2.4% |
| 2025-11-20 (26) | 2025-11-25 (19) | 5일 | −4.3% |
| 2026-03-06 (29) | 2026-04-09 (19) | 34일 | +0.0% |
| **3회 연속 복리 누적** | | | **−1.9%** |

**전략 B — 매수 VIX≥30 → 매도 VIX<20 (1회)**: 2026-03-27(31)→04-09(19), 13일, **+6.3%**

**벤치마크 — 1년 buy & hold**: **+323.8%**

![VIX 정상화 매도 왕복매매](charts/70_vix_roundtrip.png)

→ **계산은 정확하나, 앞선 +56~174%는 '신호의 힘'이 아니라 '보유기간(강세장)의 힘'이었다.** 기사 말대로 **반등 때 실제로 팔면 수익은 −2~+6%로 거의 소멸.** 이 1년에서 *공포 타이밍 자체의 단기 edge는 미미*했고, 큰 수익은 전부 **"안 팔고 오래 보유"**에서 나왔다 — 박 대표의 진짜 핵심("코어는 팔지 말 것")과 일치. 단, 극도공포(VIX≥30) 매수는 13일 +6.3%로 VIX≥25보다 우월.

### 검증 8 — ★★ 종목별: 공포에 산 케이스 vs 공포에 판 케이스
- **공포구간** = VIX 25 돌파 ~ 20 회복 (1년 3회: 10월·11월·올 3~4월).
- **① 공포매수(역발상)** = 공포구간에만 보유(사고-팔고), 평소 현금.
- **② 공포매도(회피)** = 공포구간엔 팔고 평소만 보유. (①+② 보유구간 = 1년 보유)
...
→ **공포 타이밍 자체로는 거의 못 벌었다**(① −11~+7%, 바스켓 −2%). **돈은 전부 '평소(비공포) 구간'에서 발생**(② +26~+812%). 조선·한화·현대차는 **공포 회피가 1년 보유보다 우월**(공포구간이 마이너스). 반도체만 공포매수가 소폭 플러스. → 앞선 큰 수익은 "공포에 산 효과"가 아니라 **"구조적 성장주를 안 팔고 오래 보유한 효과"**임이 종목별로 확정.
...
- **수익의 출처 = 보유기간** — 검증 7대로 반등 시 매도하면 단기 edge는 −2~+6%로 소멸. 큰 수익은 타이밍이 아닌 장기 보유(강세장)에서 발생.
...
*데이터·스크립트: `analysis/` (fetch_and_plot.py, backtest.py, vix25_analysis.py, hyspread_analysis.py, correlation_analysis.py) · 차트 20종 · 원자료 CSV.*
