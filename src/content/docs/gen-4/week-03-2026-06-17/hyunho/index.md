---
title: 임현호 - "공포의 중심에서 매수 버튼을 눌러라" 역발상 투자전략 (데이터 검증)
description: 2026-06-17 경제잡지 스터디 4기 3주차 공유 문서입니다. 박세익 체슬리투자자문 대표의 역발상 투자전략을 실제 시장 데이터로 검증합니다.
author: 임현호
date: 2026-06-17
generation: 4
week: 3
tags: [역발상투자, 공포지수, VIX, 백테스트, 코어앤새틀라이트]
keywords:
  [
    VIX,
    하이일드스프레드,
...
source:
  ...
  url:
  section: COVER STORY - 시나리오 (박세익 체슬리투자자문 대표)
entities:
  countries: [한국, 미국, 이란]
  regions: []
  organizations: [체슬리투자자문]
  companies: [삼성전자, SK하이닉스, HD한국조선해양, 한화에어로스페이스, 현대차]
  people: [박세익]
  indicators: [VIX, 하이일드스프레드, PER, PBR, KOSPI]
related:
  topics: [역발상투자, 코어앤새틀라이트, 분할매수, 차별화장세, 보유기간효과]
  pages: []
sidebar:
  label: 임현호
  order: 7
...
![코어 앤 새틀라이트 비중](./images/10_core_satellite.png)
...
![반도체](./images/11_theme_반도체.png)
![조선](./images/12_theme_조선.png)
![전력인프라](./images/13_theme_전력인프라.png)
![방산](./images/14_theme_방산.png)
![피지컬AI/완성차](./images/15_theme_피지컬AI-완성차.png)
...
![KOSPI](./images/01_kospi.png)
![VIX 공포지수](./images/02_vix.png)
![HY스프레드 프록시(HYG)](./images/03_hyg.png)
...
![코어 바스켓 vs KOSPI 추이](./images/30_backtest_value.png)
...
![3월 공포국면 매수 타이밍별 수익률](./images/31_backtest_timing.png)
![패닉셀 vs 보유 기회손실](./images/32_backtest_panic.png)
...
![VIX 정상화 매도 왕복매매](./images/70_vix_roundtrip.png)

**2-C. 종목별 — 보유일 기준 공정 비교 (시간효과 제거)**
- 두 대칭 전략: **높사낮팔**(공포에 산, 공포구간 29일 보유) vs **높팔낮사**(공포에 판, 평소 214일 보유).
- 보유일이 **7.4배** 차이 나므로 총수익은 무의미 → **보유 1거래일당 수익률**로 비교.

| 종목 | 높사낮팔 (공포매수) 일당 | 높팔낮사 (공포매도) 일당 |
|------|---:|---:|
| 삼성전자 | +0.25% | +0.81% |
| SK하이닉스 | +0.18% | +1.04% |
| HD한국조선해양 | −0.20% | +0.11% |
...
![보유 1일당 수익률 비교](./images/82_perday_compare.png)

→ 시간효과를 제거해도 **전 종목에서 '높팔낮사'(공포 회피·평소 보유)가 우월**. 즉 이 1년엔 공포 타이밍 매수가 하루당 기준으로도 손해였고, 수익은 **공포 베팅이 아니라 평소 상승추세 보유**에서 발생. (단 표본 1년·강세장 한정)
...
![VIX≥25 신호별 종목별 수익률 히트맵](./images/40_vix25_heatmap.png)
![VIX≥25 신호 평균 수익률](./images/41_vix25_avg.png)
...
![VIX vs HY스프레드 상관 시계열](./images/60_corr_timeseries.png)
![VIX vs HY스프레드 산점도](./images/61_corr_scatter.png)
...
![종목별 PER](./images/20_per.png)
![종목별 PBR](./images/21_pbr.png)
...
> 재현 스크립트·원자료(CSV)는 저장소 `analysis/` 디렉터리에 있습니다.
