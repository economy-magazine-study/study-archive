...
Claude Code, Codex처럼 코드를 직접 수정할 수 있는 AI 에이전트에서는 먼저 아래 프롬프트로 작업 환경을 준비합니다. 이미 저장소가 열려 있으면 클론을 다시 만들지 않고 현재 상태를 확인합니다.

```text
너는 경제잡지 스터디 아카이브 저장소를 수정하는 도우미야.
먼저 문서를 작성할 수 있는 작업 환경을 준비해줘.

저장소:
- GitHub 저장소: https://github.com/economy-magazine-study/study-archive
- 기본 브랜치: main

작업 순서:
1. 현재 디렉터리가 `study-archive` 저장소인지 확인한다.
2. 저장소가 없으면 아래 저장소를 클론한다.
   `git clone https://github.com/economy-magazine-study/study-archive.git`
3. 저장소가 이미 있으면 `git status`로 현재 변경사항을 확인한다.
4. 사용자가 만든 미커밋 변경사항이 있으면 절대 되돌리거나 덮어쓰지 말고, 먼저 상태를 알려준다.
5. 원격 저장소와 연결되어 있는지 `git remote -v`로 확인한다.
6. 가능하면 `git pull --ff-only`로 최신 main을 가져온다.
7. 의존성이 설치되어 있지 않으면 프로젝트의 packageManager 설정에 맞춰 설치한다.
8. 가능하면 `pnpm build`를 실행해 문서 빌드가 되는지 확인한다.

주의:
- `git reset --hard`, `git checkout --`, `rm -rf`처럼 기존 작업을 지울 수 있는 명령은 실행하지 않는다.
- 권한, 인증, 패키지 설치, 네트워크 문제로 막히면 임의로 우회하지 말고 사용자에게 필요한 조치를 알려준다.
- 환경 준비가 끝나면 현재 저장소 경로, 브랜치, 변경사항 유무, 빌드 실행 결과를 짧게 알려준다.
```
